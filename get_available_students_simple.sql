            ...
            WHERE u2.role_id = '3'
              ...
              AND NOT EXISTS (
                  ...
                  WHERE ts.teacher_id = p_teacher_id 
                    AND ts.student_id = u2.id
              )
              AND (
                  p_keyword = '' OR 
                  LOWER(u2.full_name) LIKE LOWER('%' || p_keyword || '%') OR
                  LOWER(u2.user_number) LIKE LOWER('%' || p_keyword || '%') OR
                  LOWER(u2.email) LIKE LOWER('%' || p_keyword || '%')
              )
              AND (p_grade = '' OR u2.grade = p_grade)
              AND (p_department = '' OR u2.department = p_department)
        ) as total_count
    FROM users u

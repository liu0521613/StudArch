// 简单的SQL执行脚本 (ES Module版本)
import fs from 'fs';
import path from 'path';

// 读取数据库函数SQL文件
const sqlFile = path.join(process.cwd(), 'fix_database_functions_for_real_data.sql');
...
try {
  ...
  const outputFile = path.join(process.cwd(), 'database_functions_to_execute.sql');
